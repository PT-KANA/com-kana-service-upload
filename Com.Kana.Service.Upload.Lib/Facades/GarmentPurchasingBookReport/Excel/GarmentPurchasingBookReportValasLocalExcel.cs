...
namespace Com.Kana.Service.Upload.Lib.Facades.GarmentPurchasingBookReport.Excel
{
    public static class GarmentPurchasingBookReportValasLocalExcel
    {
        public static async Task<MemoryStream> GenerateExcel(DateTimeOffset startDate, DateTimeOffset endDate, ReportDto data, int timeZone)
        {
            ...
            reportDataTable.Columns.Add(new DataColumn() { ColumnName = "DPP (IDR)", DataType = typeof(string) });
            reportDataTable.Columns.Add(new DataColumn() { ColumnName = "PPN (IDR)", DataType = typeof(string) });
            reportDataTable.Columns.Add(new DataColumn() { ColumnName = "PPh (IDR)", DataType = typeof(string) });
            ...
            using (var package = new ExcelPackage())
            {
                ...
                foreach (var columns in reportDataTable.Columns)
                {
                    ...
                    else if (column.ColumnName == "DPP (IDR)" || column.ColumnName == "PPN (IDR)" || column.ColumnName == "PPh (IDR)")
                    {
                        var rowStartHeaderSpan = rowStartHeader + 1;
                        worksheet.Cells[rowStartHeaderSpan, colStartHeader].Value = column.ColumnName;
                        worksheet.Cells[rowStartHeaderSpan, colStartHeader].Style.Border.BorderAround(OfficeOpenXml.Style.ExcelBorderStyle.Thin);
